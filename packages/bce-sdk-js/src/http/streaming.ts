...
export class Stream<Item> implements AsyncIterable<Item> {
  ...
  static fromSSEResponse(response: Response, controller: AbortController) {
    ...
    async function* iterMessages(): AsyncGenerator<ServerSentEvent, void, unknown> {
      if (!response.body) {
        controller.abort()
        throw new BCEError(`Attempted to iterate over a response with no body`)
      }
...
class LineDecoder {
  ...
  decodeText(bytes: Bytes): string {
    ...
    throw new BCEError(`Unexpected: neither Buffer nor TextDecoder are available as globals. Please report this error.`)
  }
